...
<div class="col-md-12">
	...
	<table class="table table-bordered">
		<tr>
			<td colspan="13">მოთხოვნა Exact-ის უფლებებზე</td>
		</tr>
		...
		<?php
			$query = "SELECT 
					a.id,a.res_id,c.usr_id, a.depID,d.departName,a.role_id,b.role_name,statuss,DateTo,DirAuthStatus,DirAuthResID,Request_date,ITAuthStatus,ITAuthResID,Authorized_date,Rejected,EndStatus,ActiveDate,InactiveDate
					FROM ApRights a 
					JOIN pwrole b on a.role_id = b.role_id
					JOIN humres c on a.res_id  = c.res_id
					JOIN DepartmentUser d on a.depID   = d.id
					JOIN UsersForRights f on a.depID = f.depId
					WHERE statuss = 'A' AND EndStatus = 0 AND f.res_id = '$person'";
		...
		<tr>
			<td colspan="13">
				...
				<input type="submit" name="reject" class="btn btn-danger" value="წაშლა">
			</td>
		</tr>
	</table>
	</form>
...
<div class="col-md-12">
	...
	<table class="table table-bordered">
		...
		<tr>
			<td colspan="13">
				<input type="submit" name="submit1" class="btn btn-success" value="დადასტურება">

				<input type="submit" name="reject1" class="btn btn-danger" value="წაშლა">
			</td>
		</tr>
	</table>
	</form>
...
<?php
	...
	if(isset($_POST['submit'])){
		...
		foreach ($check_id as $value) {
			$query2 = "UPDATE ApRights SET DirAuthStatus = '1',DirAuthResID = '$person',Authorized_date = '$datt' WHERE id = '$value'";
	...
	if(isset($_POST['reject'])){
		...
		foreach ($check_id as $value) {
			$query3 = "DELETE FROM ApRights WHERE id = '$value'";
	...
	if(isset($_POST['submit1'])){
		...
		foreach ($check_id1 as $value1) {
			$query8 = "UPDATE ApWhs SET DirAuthStatus = '1',DirAuthResID = '$person',Authorized_date = '$datt' WHERE id = '$value1'";
	...
	if(isset($_POST['reject1'])){
		...
		foreach ($check_id1 as $value1) {
			$query9 = "DELETE FROM ApWhs WHERE id = '$value1'";
